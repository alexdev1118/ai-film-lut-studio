...
export const generateCubeLut = ({ lutName, lutSize, adjustments, referenceAverageColor, inputColorConfig }: LutExportOptions): CubeExportResult => {
  ...
  const lines: string[] = [
    ...
    "# LUT Type: Basic creative Rec.709/display-space LUT",
    "# Not a camera Log technical conversion LUT",
    `# Input Brand: ${inputColorConfig?.brand ?? inputColorConfig?.brandId ?? "Generic"}`,
    `# Input Gamma: ${inputColorConfig?.gamma ?? "Rec.709"}`,
